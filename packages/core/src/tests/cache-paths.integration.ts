...
import { firstSubstantiveUserText } from "../utils/nested-agent";
...
async function testQwenChromeAndCursorSessions() {
  ...
  // Inbound-captured session id still wins over model/prompt fingerprint,
  // but opening user text splits workers that share that session id.
  ...
  assert.equal(
    ...
    subA,
    "opening user text splits shared session ids for every harness"
  );
  ...
  const reminderRequest = (task: string): UnifiedChatRequest => ({
    model: "composer-2",
    messages: [
      {
        role: "user",
  ...
  assert.equal(
    firstSubstantiveUserText(reminderRequest("task alpha")),
    "task alpha"
  );
  const subReminderA = buildSessionKey({
    ...parent,
    isSubagent: true,
    firstUserText: firstSubstantiveUserText(reminderRequest("task alpha")),
  });
  const subReminderB = buildSessionKey({
    ...parent,
    isSubagent: true,
    firstUserText: firstSubstantiveUserText(reminderRequest("task beta")),
  });
  const subReminderCollision = buildSessionKey({
    ...parent,
    isSubagent: true,
    firstUserText: firstSubstantiveUserText({
      model: "composer-2",
      messages: [
        {
          role: "user",
  ...
  assert.notEqual(subReminderA, subReminderB);
  assert.equal(subReminderA, subA);
  assert.notEqual(
    subReminderCollision,
    subReminderA,
    "reminder-only first user must not alias a real Task prompt"
  );
}
