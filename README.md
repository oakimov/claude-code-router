...
- **Codex-native model discovery**: Exposes an OpenAI-compatible `GET /v1/models` for the configured providers, and adds `ccr codex-config` to publish CCR-routed models into Codex's native picker — generating a Codex model catalog enriched with models.dev metadata (friendly names, context windows, reasoning effort levels) and wiring Codex's `config.toml` to route over the Responses API.
...
Discover what the router can reach with `GET /v1/models` (alias `/models`), an OpenAI-compatible list whose ids are the same `provider,model` values the chat routes accept. To surface CCR models in Codex's native picker, use `ccr codex-config`, which writes a Codex model catalog and the managed `config.toml` block that points Codex at the router.
