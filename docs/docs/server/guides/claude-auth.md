...
For **other clients**, `claude-auth` also inserts two entries at the front of the Anthropic `system` array (before whatever system text the caller supplied), matching what Claude Code itself sends:
