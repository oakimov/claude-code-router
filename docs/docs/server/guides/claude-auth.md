...
1. A billing marker text block: `x-anthropic-billing-header: cc_version=${CC_VERSION}.${suffix}; cc_entrypoint=unknown; cch=00000;` for the first-party Anthropic profile — despite its name this travels as `system[0]` text, **not** an HTTP header. `suffix` is a 3-hex-char digest derived from the first user message's text as sent (after the relocation described below, that is the caller's relocated system prompt, so it stays stable when a client drops or summarizes early turns) and the CLI version. The current `2.1.280` profile does not use the older random `cch` behavior. Neither carries `cache_control`.
...
`claude-model-catalog.ts` holds a per-model table (context window, native-1M support, max output tokens, default effort, and a `capabilities` list such as `effort`, `context_management`, `mid_conv_system`, `fast_mode`, `adaptive_thinking`) that drives both the beta synthesis above and a post-build adjustment pass (`applyClaudeModelCapabilityAdjustments`): stripping an unsupported `effort` field from `thinking`/`output_config`, shaping the `thinking` block (`adaptive` vs `enabled`; `enabled` always carries a `budget_tokens`, taken from the client or derived from the requested effort as a share of `max_tokens`, at least Anthropic's 1024 minimum and below `max_tokens`, with thinking omitted when both cannot hold), and clamping `max_tokens` to the model's known upper bound. This replaces per-model conditionals with a single table lookup, keyed by a normalized model id (CCR's `provider,` prefix, the `[1m]` marker, and Anthropic's `-YYYYMMDD` date suffix are all stripped before lookup).
...
Native Desktop and CLI cache marker placement is preserved exactly, including feature-gated scope choices. CCR never adds markers to a marker-less native request or moves markers that are present. One change applies on the `claude-auth` OAuth route: when no marker on the request sets a TTL, every ephemeral marker gets `ttl: "1h"`. Claude Code picks its TTL from how it authenticates to CCR, and with an API key (such as a gateway key) it requests the 5-minute default for every request, so idle gaps longer than five minutes would otherwise rewrite the whole conversation cache. A request that already sets a TTL on any marker is forwarded as sent. Only real marker positions are considered (top level, tools, system blocks, message content blocks and `tool_result` content); values inside tool inputs are user data and are left alone. One-hour cache writes bill at a higher rate than 5-minute writes; see [Claude Code prompt caching](https://code.claude.com/docs/en/prompt-caching#cache-lifetime). To keep the client's own TTL instead, set `"CLAUDE_AUTH_NATIVE_CACHE_TTL": "client"` in `config.json` (default `"1h"`). Direct `Anthropic` API-key routes keep the client's markers unchanged. Current Desktop 3P conversations run through Desktop's bundled Agent SDK and can therefore author Claude Code-shaped system/message breakpoints. Only the `other` path authors cache fields, using the 2.1.280 profile: the billing block remains unmarked, cacheable system blocks receive the profile's cache control, the relocated caller system prompt receives the breakpoint Claude Code places on its own system prompt (so tools and instructions stay cached when a client rewrites or windows its history), and one message tail receives the final breakpoint. When there is no user message to relocate into, only the first and last cacheable system blocks are marked, keeping the request within Anthropic's four breakpoints; parts the body builder does not send (empty text, an image without a URL, a file without data) never carry the tail breakpoint, and when the final turn sends nothing at all (the builder omits it) the tail breakpoint moves to the previous eligible message. When the tail is a tool result, the breakpoint sits on the `tool_result` block, as in Claude Code. There is no universal CCR cache normalizer.

Earlier releases prepended the relocated prompt to the first user message's text instead of giving it its own block. The billing suffix was also derived from the first user message before relocation. Conversations already in flight when CCR is upgraded therefore miss the prompt cache once; the following turn is cached again.
